...
class LedgerControllerTest {
  ...
  void deposit_Success() throws Exception {
    ...
    doNothing().when(ledgerService)
        .deposit(any(AccountNumber.class), any(UUID.class), any(BigDecimal.class), anyString());
    ...
    verify(ledgerService, times(1)).deposit(any(AccountNumber.class), any(UUID.class),
        eq(new BigDecimal("100.00")), eq("Test deposit"));
  }

  @Test
  void deposit_InvalidTransferId() throws Exception {
    String accountId = "26082099";
    TransferRequest request = new TransferRequest("invalid-uuid", new BigDecimal("100.00"),
        "Test deposit");
  ...
  void deposit_AmountLessThanMinimum() throws Exception {
    String accountId = "26082099";
    String transferId = UUID.randomUUID().toString();
    TransferRequest request = new TransferRequest(transferId, new BigDecimal("0.50"),
        "Test deposit");
  ...
  void deposit_DuplicateTransaction() throws Exception {
    String accountId = "26082099";
    String transferId = UUID.randomUUID().toString();
    TransferRequest request = new TransferRequest(transferId, new BigDecimal("100.00"),
        "Test deposit");
  ...
  void withdraw_Success() throws Exception {
    ...
    TransferResponse response = new TransferResponse(tfID.toString(), amount.toString(),
        "Withdrawal success");
  ...
  void withdraw_InsufficientFunds() throws Exception {
    ...
    String transferId = UUID.randomUUID().toString();
    TransferRequest request = new TransferRequest(transferId, new BigDecimal("1000.00"),
        "Test withdrawal");

    doThrow(new InsufficientFundsException("Insufficient funds for withdrawal " + transferId))
        .when(ledgerService)
        .withdraw(any(AccountNumber.class), any(UUID.class), any(BigDecimal.class), anyString());
  ...
  void withdraw_InvalidTransferId() throws Exception {
    String accountId = "ACC001";
    TransferRequest request = new TransferRequest("not-a-uuid", new BigDecimal("50.00"),
        "Test withdrawal");
  ...
  void txHistory_Success() throws Exception {
    ...
    List<Transaction> transactions = List.of(
        new Transaction(accountId, "100.00", TransferType.DEPOSIT, LocalDateTime.now(),
            "Deposit 1"),
        new Transaction(accountId, "50.00", TransferType.WITHDRAW, LocalDateTime.now(),
            "Withdrawal 1")
    );
